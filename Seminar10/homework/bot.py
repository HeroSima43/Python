...
def start(update, _):
    reply_keyboard = [['Операции с рациональными числами', 'Операции с комплексными числами', 'Cancel']]
...
def choice(update, context):
    ...
    logger.info('%s: Выбор операции: %s', user.first_name, update.message.text)
...
def rational_one(update, context):
    user = update.message.from_user
    logger. info("Пользователь %s: ввел число: %s", user.first_name, update.message.text)
...
def operations_rational(update, context):
    user = update.message.from_user
    logger.info(
        'Пользователь %s выбрал операцию: %s', user.first_name, update.message.text)
...
def complex_one(update, context):
    user = update.message.from_user
    logger.info(
        'Пользователь %s: ввел число %s', user.first_name, update.message.text)
    user_choice = update.message.text
...
if __name__ == "__main__":
    ...
    # Определяем обработчик разговоров 'ConsersationHandler'
